def call(Map config=[:], Closure body) {
    ...
    properties([parameters([choice(choices: time_options, description: description_start_time, name: 'EC2_start_time')])])
    properties([parameters([booleanParam(defaultValue: true, description: 'test boolean', name: 'RunningNonStop'), string(defaultValue: 'TBD', description: 'not sure', name: 'not_used', trim: false)])])
    properties([parameters([string(defaultValue: 'TBD', description: 'not sure', name: 'not_used', trim: false)])])
